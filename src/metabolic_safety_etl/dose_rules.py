...
def extract_dose_rule_facts(facts: Iterable[EvidenceFact]) -> list[EvidenceFact]:
    ...
    Only explicit maximum / not-exceed wording is promoted. Routine dose ranges
    remain dose_candidate evidence because they are not overdose thresholds.
    """
    ...
    for subject_id in sorted(set(source_texts) | set(candidates)):
        base_subject = normalize_label_subject(subject_id)
        if subject_id in existing_subjects or base_subject in existing_subjects:
            continue
        group = [*source_texts.get(subject_id, []), *candidates.get(subject_id, [])]
        if not group:
            continue
        evidence_text = "\n".join(fact_text(fact) for fact in group)
        daily = best_match(evidence_text, DAILY_PATTERNS, "window") or best_candidate_match(candidates.get(subject_id, []), "window")
        single = best_match(evidence_text, SINGLE_PATTERNS, "single") or best_candidate_match(candidates.get(subject_id, []), "single")
        if not daily and not single:
            continue
        first = strongest_source(group)
        generated.append(make_dose_rule_fact(base_subject, subject_id, daily, single, first, evidence_text))
    return generated
...
def make_dose_rule_fact(base_subject: str, original_subject: str, daily: DoseMatch | None, single: DoseMatch | None, first: EvidenceFact, evidence_text: str) -> EvidenceFact:
    ...
    return EvidenceFact(
        ...
        claim={
            ...
            "window_hours": 24,
            "thresholds": thresholds,
            "basis": "adult_or_unspecified_label_ceiling",
        ...
        extraction_method="regex_label_dose_rule_v2",
        review_status="unreviewed",
