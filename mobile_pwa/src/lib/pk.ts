import type { JournalEntry, SubstanceBundle, SubstanceDetail, UserProfile } from "../types";
...
export function adjustedPkParams(entry: JournalEntry, substance?: SubstanceDetail, profile: UserProfile = defaultProfile) {
  ...
  const halfLife = Number(substance?.base_half_life || 4);
  const baseHalfLifeHours = Number.isFinite(halfLife) && halfLife > 0 ? halfLife : 4;
...
export function activeEntries(entries: JournalEntry[], bundles: Record<string, SubstanceBundle>, profile: UserProfile, now = Date.now()) {
  return entries.filter((entry) => {
    const params = adjustedPkParams(entry, bundles[entry.substanceId]?.detail, profile);
    const windowMinutes = Math.max(params.adjustedHalfLifeHours * 6 * 60, Number(bundles[entry.substanceId]?.detail.base_duration || 360), 60);
...
export function entryHasMeaningfulExposure(entry: JournalEntry, bundles: Record<string, SubstanceBundle>, profile: UserProfile, now = Date.now(), horizonHours = 24) {
  const substance = bundles[entry.substanceId]?.detail;
